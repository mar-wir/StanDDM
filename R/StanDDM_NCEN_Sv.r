StanDDM_NCEN_Sv <- function(simulation, modal, data, control, seed, warmup, num_cores, iter){
    
    model_name <- paste('StanDDM_NCEN_Sv', modal, sep="") #modal assigned in main
    ...
    if(simulation){
        ...
        fakeParams <- makeFakeParams(nsub = 10, include = c('sv'))
    ...
    model {
        ...
        RTu[i, 1:Nu[i]] ~ wiener(alpha_mu[i], tau_mu[i], beta_mu[i], delta[NuL:NuU]);
    
        NlL = sum(Nl[1:i]) -Nl[i] +1 + sum(Nu);
        NlU = sum(Nl[1:i]) + sum(Nu);
    
        RTl[i, 1:Nl[i]] ~ wiener(alpha_mu[i], tau_mu[i], 1-beta_mu[i], -delta[NlL:NlU]);
    ...
    generated quantities {
        ...
        for (i in 1:N) {
            ...
            for (j in NuL:NuU){
                log_lik[j] = wiener_lpdf(RTu[i, 1:Nu[i]] | alpha_mu[i], tau_mu[i], beta_mu[i], delta[j]);
            }
            ...
            for (j in NlL:NlU){
                log_lik[j] = wiener_lpdf(RTl[i, 1:Nl[i]] | alpha_mu[i], tau_mu[i], 1-beta_mu[i], -delta[j]);
            }
        }
    }
    "
    ...
    pars <- c("delta", 'delta_sigma', "alpha_mu", "beta_mu", "delta_mu", "tau_mu", 'log_lik')
    ...
    #----------------------------------------------------------------- 
        ...
        names(extracted_params) <- c('sv','a', 'z', 'v', 't') #dependent on model
