StanDDM_NCEN_Sv_Sz <- function(simulation, modal, data, control, seed, warmup, num_cores, iter){
        
    model_name <- paste('StanDDM_NCEN_Sv_Sz', modal, sep="") #modal assigned in main
    ...
    if(simulation){
        ...
        fakeParams <- makeFakeParams(nsub = 10, include = c('sv', 'sz'))     
    ...
    transformed parameters {
        ...
        for (i in 1:N) {//begin subject loop
            ...
            beta[NuL:NuU] = Phi_approx(beta_mu[i] + beta_sigma[i] * beta_pr[NuL:NuU]);
            beta[NlL:NlU] = Phi_approx(beta_mu[i] + beta_sigma[i] * beta_pr[NlL:NlU]);        
        }//end subject loop
    ...
    model {
        ...
        for (i in 1:N) {
            ...
            RTu[i, 1:Nu[i]] ~ wiener(alpha_mu[i], tau_mu[i], beta[NuL:NuU], delta[NuL:NuU]);
            
            NlL = sum(Nl[1:i]) -Nl[i] +1 + sum(Nu);
            NlU = sum(Nl[1:i]) + sum(Nu);
            
            RTl[i, 1:Nl[i]] ~ wiener(alpha_mu[i], tau_mu[i], 1-beta[NlL:NlU], -delta[NlL:NlU]);
    ...
    generated quantities {
        ...
        for (i in 1:N) {
            ...
            for (j in NuL:NuU){
                log_lik[j] = wiener_lpdf(RTu[i, 1:Nu[i]] | alpha_mu[i], tau_mu[i], beta[j], delta[j]);
            }
            ...
            for (j in NlL:NlU){
                log_lik[j] = wiener_lpdf(RTl[i, 1:Nl[i]] | alpha_mu[i], tau_mu[i], 1-beta[j], -delta[j]);
            }
        }
    }
    "
    ...
    pars <- c('delta_sigma', 'beta_sigma', "alpha_mu", 'beta_mu', "delta_mu", "tau_mu", "log_lik")
    ...
    #----------------------------------------------------------------- 
        ...
        names(extracted_params) <- c('sv', 'sz', 'a', 'z', 'v', 't') #dependent on model
