StanDDM_NCEN_St_Sz <- function(simulation, modal, data, control, seed, warmup, num_cores, iter){
    
    model_name <- paste('StanDDM_NCEN_St_Sz', modal, sep="") #modal assigned in main
    ...
    if(simulation){
        ...
        fakeParams <- makeFakeParams(nsub = 10, include = c('st', 'sz'))   
    ...
    parameters {
      ...
      //// Subject-level raw parameters (for de-centering)
    ...
    model {
        ...
        for (i in 1:N) {
            ...
            RTu[i, 1:Nu[i]] ~ wiener(alpha_mu[i], tau[NuL:NuU], beta[NuL:NuU], delta_mu[i]);
        
            NlL = sum(Nl[1:i]) -Nl[i] +1 + sum(Nu);
            NlU = sum(Nl[1:i]) + sum(Nu);
        
            RTl[i, 1:Nl[i]] ~ wiener(alpha_mu[i], tau[NlL:NlU], 1-beta[NlL:NlU], -delta_mu[i]);
    ...
    generated quantities {
        ...
        for (i in 1:N) {
            ...
            for (j in NuL:NuU){
                log_lik[j] = wiener_lpdf(RTu[i, 1:Nu[i]] | alpha_mu[i], tau[j], beta[j], delta_mu[i]);
            }
            ...
            for (j in NlL:NlU){
                log_lik[j] = wiener_lpdf(RTl[i, 1:Nl[i]] | alpha_mu[i], tau[j], 1-beta[j], -delta_mu[i]);
            }
        }
    }
    "
    ...
    pars <- c('tau_sigma', "beta_sigma", "alpha_mu", "beta_mu", "delta_mu", "tau_mu", 'log_lik')
    ...
    #----------------------------------------------------------------- 
        ...
        names(extracted_params) <- c('st', 'sz', 'a', 'z', 'v', 't') #dependent on model
